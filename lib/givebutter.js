...
async function upsertContactByEmail(
  apiKey,
  { email, firstName, lastName, phone, externalId, tags, address1, address2, city, state, zipcode, country }
) {
  if (!email) {
  ...
  const payload = {
    ...
    ...(address1 ? { primary_address_1: address1 } : {}),
    ...(address2 ? { primary_address_2: address2 } : {}),
    ...(city ? { primary_city: city } : {}),
    ...(state ? { primary_state: state } : {}),
    ...(zipcode ? { primary_zipcode: zipcode } : {}),
